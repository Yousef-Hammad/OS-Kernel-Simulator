...
int total_wait_time = 0, total_turnaround_time = 0, process_count = 0;
...
*/
proc_t create_proc(int pid, int arrival_time, int total_cpu_time, int io_frequency, int io_duration, int memory){
    process_count++;
...
void print_nodes(node_t head) {
    ...
    while (current != NULL) {
        ...
        if (p->partition == NULL) {
            printf("Current partition: none\n");
        } else {
            printf("Current partition: %iMb\n", p->partition->size);
        }
...
/* FUNCTION DESCRIPTION: print_memory_partitions
* Prints all of the memory partitions and their availability
*/
void print_memory_partitions(struct memory_partition partitions[]) {
    for (int i = 0; i < PARTITION_COUNT; i++) {
        struct memory_partition p = partitions[i];
        if (p.free) {
            printf("Partition %d(%dMb): Free\n", i + 1, p.size);
        } else {
            printf("Partition %d(%dMb): %dMb/%dMb In Use\n", i + 1, p.size, p.space_used, p.size);
        }
    }
}

/* FUNCTION DESCRIPTION: print_memory_info
* Prints info about memory partitions and usage 
*/
void print_memory_info(struct memory_partition partitions[]) {
    int total_memory = 0;
    int total_used_memory = 0;
    int total_free_memory = 0;
    int total_usable_memory = 0;

    for (int i = 0; i < PARTITION_COUNT; i++) {
        struct memory_partition p = partitions[i];
        total_memory += p.size;
        total_free_memory += p.size - p.space_used;
        total_used_memory += p.space_used;
        if (p.free) total_usable_memory += p.size;
    }
    printf("Memory Partition Info:\n");
    printf("Total Memory Free: %dMb/%dMb\n", total_free_memory, total_memory);
    printf("Total Memory Used: %dMb/%dMb\n", total_used_memory, total_memory);
    printf("Total Usable Memory: %dMb\n", total_usable_memory);
    print_memory_partitions(partitions);
    printf("\n");
}
...
node_t read_proc_from_file(char *input_file){
    ...
    proc_t proc;
    int pid, arrival_time, total_cpu_time, io_frequency, io_duration, memory;
    ...
    while (fgets(row, MAXCHAR, f)){
        ...
        proc = create_proc(pid, arrival_time, total_cpu_time, io_frequency, io_duration, memory);
...
/* FUNCTION DESCRIPTION: record_process_ready
* This function updates metrics based on a process entering the ready queue
* The parameters are: 
*    - clock: the current cpu clock cycle
*/
void record_process_ready(int clock){
    total_wait_time -= clock;
}

/* FUNCTION DESCRIPTION: record_process_exited_ready
* This function updates metrics based on a process exiting the ready queue
* The parameters are: 
*    - clock: the current cpu clock cycle
*/
void record_process_exited_ready(int clock){
    total_wait_time += clock;
}

/* FUNCTION DESCRIPTION: record_new_process_entered
* This function updates metrics based on a new process entering
* The parameters are: 
*    - clock: the current cpu clock cycle
*/
void record_new_process_entered(int clock){
    total_turnaround_time -= clock;
}

/* FUNCTION DESCRIPTION: record_process_terminated
* This function updates metrics based on a process terminating
* The parameters are: 
*    - clock: the current cpu clock cycle
*/
void record_process_terminated(int clock){
    total_turnaround_time += clock;
}

/* FUNCTION DESCRIPTION: allocate_partition
* This function attempts to assign a memory partition to a given process
* The parameters are: 
*    - new_list: The list of new processes waiting to enter the ready queue
* The return value indicates if the process was successfully allocated
*/
int allocate_partition(node_t new_list, struct memory_partition partitions[]) {
    for (int i = 0; i < PARTITION_COUNT; i++) {
        if (partitions[i].free && new_list->p->memory <= partitions[i].size) {
            partitions[i].free = 0;
            partitions[i].space_used = new_list->p->memory;
            new_list->p->partition = &partitions[i];

            // Return true if a partition was allocated
            return 1;
        }
    } 

    // Return false if no partitions are available
    return 0;
}
...
int main( int argc, char *argv[]) {
    ...
    node_t running = NULL;
    char *input_file;
    int verbose, alternate_partitions;

    if(argc == 2){
        input_file = argv[1];
        verbose = 0;
        alternate_partitions = 0;
    } else if( argc == 3 ) {
        input_file = argv[1];
        verbose = atoi(argv[2]);
        alternate_partitions = 0;
        ...
        input_file = argv[1];
        verbose = atoi(argv[2]);
        alternate_partitions = atoi(argv[3]);
    } else {
        printf("Two, three, or four args expected.\n");
        return -1;
    }

    struct memory_partition partitions[PARTITION_COUNT] = {
        {500, 1, 0},
        {250, 1, 0},
        {150, 1, 0},
        {100, 1, 0}
    ...
    if(alternate_partitions) {
        partitions[0].size = 300;
        partitions[1].size = 300;
        partitions[2].size = 350;
        partitions[3].size = 50;
    }
    ...
    do {
        ...
        while(node != NULL){
            ...
            if(node->p->io_time_remaining <= 0){
                record_process_ready(cpu_clock);
        ...
        while(node!= NULL) {
            ...
            if(node->p->arrival_time <= cpu_clock){
                if (allocate_partition(node, partitions)) {
                    record_process_ready(cpu_clock);
                    record_new_process_entered(cpu_clock);
        ...
        if(running == NULL){
            ...
            if(ready_list!=NULL){
                record_process_exited_ready(cpu_clock);
        ...
        // Check if blocked processes can run or update arrival time of blocked processes
        node = new_list;
        while(node!= NULL) {
            if (node->p->blocked) {
                node->p->blocked = 0;
                ...
                if (allocate_partition(node, partitions)) {
                    ...
                    node->p->arrival_time += next_step ? next_step : 1;
                }
            }
            node = node->next;
        } 
        ...
        if(verbose){
            ...
            print_nodes(running);
            printf("-------------------------------\n");
            printf("The Memory Partitions are:\n");
            print_memory_partitions(partitions);
            printf("-------------------------------\n");
    ...
    if(verbose) printf("Total Wait Time (ms), Avg Wait Time (ms), Avg Turnaround Time (ms), Throughput (process/s)\n");
    if(verbose) printf("%d,", total_wait_time);
    if(verbose) printf("%.2f,", ((double) total_wait_time) / ((double) process_count));
    if(verbose) printf("%.2f,", ((double) total_turnaround_time) / ((double) process_count));
    if(verbose) printf("%.2f\n", ((double) process_count) / ((double) cpu_clock) * 1000);
