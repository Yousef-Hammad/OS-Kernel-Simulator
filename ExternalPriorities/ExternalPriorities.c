...
node_t push_node(node_t head, node_t temp){
    ...
    if(head == NULL){
        ...
        // Iterate through the list to add the new node at the end
...
/* FUNCTION DESCRIPTION: push_node_priority
* This function adds a node to the list in order of priority(as though its a queue).
* The parameters are: 
*    -head points to the head in the list
*    -temp is the node to be added
* The return value is a pointer to the list
*/
node_t push_node_priority(node_t head, node_t temp){
    // prev will be used to itterate through the list
    node_t prev;
    ...
    if(head == NULL){
        head = temp;     
    // If the new node has highest priority, make it the new head
    } else if (temp->p->priority < head->p->priority) {
        temp->next = head;
        head = temp;
    } else {
        // Iterate through the list to add the new node
        // The last node always points to NULL, so we get the next nodes until this happens
        prev = head;

        while(prev->next != NULL && (temp->p->priority >= prev->next->p->priority)){
            prev = prev->next;
        }

        // Insert the new node after the higher priority node
        temp->next = prev->next;
        prev->next = temp;
    }
    return head;
...
int main( int argc, char *argv[]) {
    ...
    do {
        ...
        record_process_ready(cpu_clock);
                ...
                ready_list = push_node_priority(ready_list, node);
