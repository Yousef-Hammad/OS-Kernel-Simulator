...
*/
int get_time_to_next_event(int cpu_clock, node_t running, node_t new_list, node_t waiting_list, int round_robin_timer){
    node_t temp;
    int next_exit=INT_MAX, next_block=INT_MAX, next_arrival=INT_MAX, next_io=INT_MAX;

    if(running != NULL){
        next_exit = running->p->cpu_time_remaining;
        next_block = min(running->p->io_time_remaining, round_robin_timer); // <--- UPDATED HERE ###############
    }
...
int main( int argc, char *argv[]) {
    ...
    do {
        ...
        record_process_terminated(cpu_clock);
            ...
            else if (round_robin_timer <= 0){
                // Time quantum expired, move the process to the ready queue
        ...
        next_step = get_time_to_next_event(cpu_clock, running, new_list, waiting_list, round_robin_timer);
